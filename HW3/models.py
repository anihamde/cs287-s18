...
class AttnCNN(nn.Module):
    def __init__(self, word_dim=300, n_layers=1, hidden_dim=500, word2vec=False,
        ...
        if self.n_layers > 1:
            self.c3_seq_enc = nn.Sequential(*[ a for b in tuple( tuple((nn.ReplicationPad2d((0,0,1,1)),nn.Dropout(LSTM_dropout),nn.Conv2d(self.hidden_dim,self.hidden_dim*2,kernel_size=(3,1),padding=(1,0)),nn.GLU(1))) for _ in range(1,self.n_layers) ) for a in b ])
            self.c3_seq_dec = nn.Sequential(*[ a for b in tuple( tuple((nn.ReplicationPad2d((0,0,2,0)),nn.Dropout(LSTM_dropout),nn.Conv2d(self.hidden_dim,self.hidden_dim*2,kernel_size=(3,1),padding=(2,0)),nn.GLU(1))) for _ in range(1,self.n_layers) ) for a in b ])            
        self.embedding_de = nn.Embedding(len(DE.vocab), word_dim)
        self.embedding_en = nn.Embedding(len(EN.vocab), word_dim)
        if word2vec:
            self.embedding_de.weight.data.copy_(DE.vocab.vectors)
